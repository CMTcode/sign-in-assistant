...
import top.xmlsj.signin.script.bilbil.apiquery.oftenAPI;
...
public class ChargeMe implements SigninTask {
    ...
    public void run() {
        ...
        String configChargeUserId = Config.chargeForLove;

        //B币券余额
        double couponBalance;
        //大会员类型
        int vipType = TaskInfoHolder.queryVipStatusType();
        ...
        if (!Boolean.TRUE.equals(Config.monthEndAutoCharge)) {
        ...
        if (TaskInfoHolder.userInfo != null) {
            couponBalance = TaskInfoHolder.userInfo.getWallet().getCoupon_balance();
        } else {
         ...
         */
        if (day >= 2 && couponBalance >= 2 &&
                Boolean.TRUE.equals(Config.monthEndAutoCharge)) {
            ...
            int resultCode = jsonObject.get(TaskInfoHolder.STATUS_CODE_STR).getAsInt();
            if (resultCode == 0) {
