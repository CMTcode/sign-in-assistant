...
public class ListenToSongsTask implements SigninTask {
    ...
    public void listenSongsDaily(MusicUser user) {
        if (user.getLevel() < 10) {
            ...
            int successCount = 0;
            int nowPlayCount = Integer.parseInt(api.userLevel(user.getToken(), user.getCookie()).getJSONObject("data").getString("nowPlayCount"));
...
//        }
            //获取每日推荐歌单，保证是不影响听歌口味
            JSONObject resourceJSON = api.getRecommentSongs(user.getToken(), user.getCookie());
            //解析歌单，返回一个map，key是歌单id,value是歌单中歌曲id的集合
            Map<Long, List<Long>> playListMap = api.parsePlayList(resourceJSON, user.getToken(), user.getCookie());
            ...
            for (Long sourceId : playListKeySet) {
                ...
                for (Long songId : songIdList) {
                    try {
                        JSONObject listenResult = api.listenSong(user.getToken(), user.getCookie(), songId + "", sourceId + "");
                        if (listenResult != null && listenResult.getInteger("code") == 200) {
                            ...
                            if (successCount >= 300 && successCount % 50 == 0) {
                                int t = Integer.parseInt(api.userLevel(user.getToken(), user.getCookie()).getJSONObject("data").getString("nowPlayCount"));
