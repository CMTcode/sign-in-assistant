...
public class GiveGift implements Task {
    ...
    public void run() {
        try {
            /* 从配置类中读取是否需要执行赠送礼物 */
            if (!Boolean.TRUE.equals(Config.giveGift)) {
    ...
    public JsonArray xliveGiftBagList() {
        JsonObject tempObject = HttpUtil.doGet("https://api.live.bilibili.com/xlive/web-room/v1/gift/bag_list");
        log.info("B站直播获取背包礼物");
        JsonElement temp = tempObject.get("data").getAsJsonObject().get("list");
        if (null == temp || temp.isJsonNull()) {
            log.info("背包礼物为null，跳过处理");
            return null;
        }
        JsonArray tempArray = temp.getAsJsonArray();
        return tempArray;
    }
